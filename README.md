...
pipe.load_lora_weights("/path/to/RGB2IR/models")
pipe.fuse_lora()
...
out = pipe(
    prompt="turn the visible image of Marine Vessel into sks infrared",
    image=[grayscale_pil_image],   # input PIL.Image, mode L or RGB
...
|----------|------|------------------|
| `data/ground_truth_ir/` | real IR (mode L) | sourced from `红外船舶数据库` (25k raw IR), filtered with **Qwen 3.5-27B** for clarity (`is_clear=True`), ranked by Laplacian variance (sharpness), random-split into 250 train + 25 val + 25 test per category at 256×256, then p2/p98 contrast-stretched |
| `data/synthetic_grayscale/` | grayscale conditioning (mode L) | the same IR images converted to grayscale (`PIL .convert("L")`) and contrast-stretched — this is what the LoRA's `cond_image` channel was trained on |
| `data/synthetic_rgb/` | RGB visualization (mode RGB) | generated by feeding real IR through **FLUX.2-Klein-9B** with category-specific prompts (steps=20, scale=1.0). Reference output of Step 1, stored for visualization — not used during LoRA training |
