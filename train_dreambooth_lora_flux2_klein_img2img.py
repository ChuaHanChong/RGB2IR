...
# check_min_version("0.38.0.dev0")  # All Flux2 classes available in 0.37.0
...
def parse_args(input_args=None):
    ...
    if args.cond_image_column is None:
        ...
        # caption_column is optional; if None, instance_prompt is used for all images
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            if os.path.isdir(args.dataset_name) and os.path.exists(os.path.join(args.dataset_name, "dataset_dict.json")):
...
def main(args):
    ...
    if precompute_latents:
        # Temporarily move transformer to CPU during caching to free GPU memory for VAE/text_encoder
        if args.offload and not is_fsdp:
            transformer.to("cpu")
            free_memory()
        prompt_embeds_cache = []
        text_ids_cache = []
    ...
    # Move transformer back to GPU after caching phase
    if args.offload and not is_fsdp:
        transformer.to(**transformer_to_kwargs)
        free_memory()
